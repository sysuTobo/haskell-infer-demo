...
__global__ void gdn_delta_rule_decode_kernel(
    ...
    // Step 3: S[j][i] = alpha * S[j][i] + beta * k[j] * delta (STATE UPDATE)
    for (int j = 0; j < k_head_dim; j++) {
        float kj = __bfloat162float(k_ptr[j]);
        S[j * v_head_dim + i] = a * S[j * v_head_dim + i] + b * kj * delta;
    }

    // Step 4: o[i] = (q @ S_new)[i] -- OUTPUT AFTER STATE UPDATE
    // Verified: PyTorch GDN sublayer gives non-zero output for first token
    float o = 0.0f;
    for (int j = 0; j < k_head_dim; j++) {
