...
# - decode-compatible final-state layout contract [H, K, V]
...
def gdr_prepare_qkv_gbeta_qwen35_kernel(
    ...
    beta_val = (1.0 / (1.0 + tl.exp(-b_val))).to(tl.bfloat16).to(tl.float32)
...
def gdr_solve_tril_64_qwen35_kernel(
    ...
    b_ai_11 = -tl.where(lower, tl.load(p_a_11, boundary_check=(0, 1), padding_option="zero").to(tl.float32), 0.0)
    b_ai_22 = -tl.where(lower, tl.load(p_a_22, boundary_check=(0, 1), padding_option="zero").to(tl.float32), 0.0)
    b_ai_33 = -tl.where(lower, tl.load(p_a_33, boundary_check=(0, 1), padding_option="zero").to(tl.float32), 0.0)
    b_ai_44 = -tl.where(lower, tl.load(p_a_44, boundary_check=(0, 1), padding_option="zero").to(tl.float32), 0.0)
    ...
    b_a_21 = tl.load(p_a_21, boundary_check=(0, 1), padding_option="zero").to(tl.float32)
    b_a_31 = tl.load(p_a_31, boundary_check=(0, 1), padding_option="zero").to(tl.float32)
    b_a_32 = tl.load(p_a_32, boundary_check=(0, 1), padding_option="zero").to(tl.float32)
    b_a_41 = tl.load(p_a_41, boundary_check=(0, 1), padding_option="zero").to(tl.float32)
    b_a_42 = tl.load(p_a_42, boundary_check=(0, 1), padding_option="zero").to(tl.float32)
    b_a_43 = tl.load(p_a_43, boundary_check=(0, 1), padding_option="zero").to(tl.float32)
...
def gdr_recompute_w_u_qwen35_kernel(
    ...
    beta = tl.load(p_beta, boundary_check=(0,), padding_option="zero")
    ...
    ai = tl.load(p_ai, boundary_check=(0, 1), padding_option="zero")
    ...
    g = tl.exp(tl.load(p_g, boundary_check=(0,), padding_option="zero"))
...
def gdr_chunk_state_qwen35_kernel(
    ...
    h_lo = tl.load(p_h0_lo, boundary_check=(0, 1), padding_option="zero").to(tl.float32)
    h_hi = tl.load(p_h0_hi, boundary_check=(0, 1), padding_option="zero").to(tl.float32)
    ...
    for chunk_idx in range(num_chunks):
        ...
        w_lo = tl.load(p_w_lo, boundary_check=(0, 1), padding_option="zero")
        w_hi = tl.load(p_w_hi, boundary_check=(0, 1), padding_option="zero")
        v_new = tl.load(p_u, boundary_check=(0, 1), padding_option="zero").to(tl.float32)
        ...
        k_lo = tl.load(p_k_lo, boundary_check=(0, 1), padding_option="zero")
        k_hi = tl.load(p_k_hi, boundary_check=(0, 1), padding_option="zero")
...
def gdr_chunk_o_qwen35_kernel(
    ...
    for k_block in range(0, KEY_DIM, BLOCK_K):
        ...
        k = tl.load(p_k, boundary_check=(0, 1), padding_option="zero")
        h = tl.load(p_h, boundary_check=(0, 1), padding_option="zero")
    ...
    v_new = tl.load(p_v, boundary_check=(0, 1), padding_option="zero")
